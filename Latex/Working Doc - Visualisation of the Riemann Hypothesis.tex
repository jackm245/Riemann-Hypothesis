...
\begin{algorithm}
    ...
    \begin{algorithmic}
        \Function{add}{$a$, $b$}\\
             \hspace{\algorithmi}indent}\Return $a + b$
        \EndFunction
    \end{algorithmic}
\end{algorithm}
...
\begin{algorithm}[ht]
    ...
    \begin{algorithmic}
        \Function{binom}{n, k}
            \State{v = 1}
            \For {i = 0 \textbf{to} k}
                \State {$\text{v} = \text{v} \times \frac{\text{n}-\text{i}}{\text{i}+1}$}
            \EndFor
        \EndFunction
    \end{algorithmic}
\end{algorithm}
...
We are then able to convert both the Zeta Function Pseudocode and the Binomial Coefficient Pseudocode into Python
